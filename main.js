...
function parseSnapshotObject(fd, self) {
  ...
  while (offset < READ_BUF_SIZE) {
    ...
    for (let i = 0; i < READ_BUF_SIZE; i++, offset++) {
      if (read_buf[i] === 0x0a) {
        self.snapshot = JSON.parse(snapshot_string + read_buf.slice(0, i - 1));
        return offset;
      }
    }
...
function parseNumericArray(fd, offset, self, name) {
...
function parseStringsBuffer(fd, offset, self, name) {
  const ret = [];
  ...
  self[name] = concatArray(ret);
  return end_last_word;
}
...
function DataAccessor(file_path) {
  ...
  offset = parseSnapshotObject(fd, this);
  ...
  // jump past '"nodes":['
  offset = parseNumericArray(fd, offset + 10, this, 'nodes');
  ...
  // jump past '"edges":['
  offset = parseNumericArray(fd, offset + 10, this, 'edges');
  ...
  // right now can only handle 2GB (on x64) of strings. which is more than the
  // v8 heap will allow. so not worried.
  const strings_start = offset;
  offset = parseStringsBuffer(fd, strings_start, this, '_strings_indexes');
